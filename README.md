...
curl -L https://github.com/0xNetuser/flux-panel/releases/download/1.8.0/docker-compose.yml -o docker-compose.yml
```
...
services:
  gost-node:
    image: 0xnetuser/gost-node:1.8.0
...
- **Xray 版本远程切换**：节点管理页面新增版本切换按钮，管理员可从面板远程升级/降级节点上的 Xray 版本
- **异步下载替换**：节点从 GitHub Releases 下载指定版本，自动备份旧版本、替换、重启，支持失败自动回滚
- **实时版本显示**：节点列表 Xray 版本从 WebSocket 实时缓存读取，切换后自动刷新
