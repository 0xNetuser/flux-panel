...
func worker(id int, args []string, ctx *context.Context, ret *int) {
	...
	cmd.Env = append(os.Environ(), fmt.Sprintf("_SVC_ID=%d", id))
...
func main() {
	...
	wsReporter := socket.StartWebSocketReporterWithConfig(config.Addr, config.Secret, config.Http, config.Tls, config.Socks, nodeVersion, config.UseTLS, config.XrayBin, config.XrayCfg)
...
// GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o node
// upx --best --lzma node
